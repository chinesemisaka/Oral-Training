...
生产必须在 HTTPS 反向代理后运行并启用：`PRODUCTION=true`、`AUTH_MODE=wechat`、HTTPS `ALLOWED_ORIGIN`、`REQUIRE_HTTPS=true`、非空 `TRUSTED_PROXY_IPS`、`ALLOW_RUNTIME_API_KEY=false`。程序只接受可信代理提供的转发协议和客户端地址，从右向左剥离代理链后执行 IP 限流；配置降级或格式错误时拒绝启动。程序添加请求 ID、JSON 结构日志、用户/IP 滑动窗口限流和健康任务计数。运维应对 `deadJobs > 0` 告警。
